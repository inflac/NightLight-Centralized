...
class NightlineStatus(db.Model):
    ...
    nightline = db.relationship(
        "Nightline", back_populates="nightline_statuses")
    status_id = db.Column(
        db.Integer,
    ...
    def add_new_status_for_all_nightlines(cls, status: "Status") -> bool:
        ...
        try:
            ...
            db.session.rollback()
            logger.error(
                f"Error creating NightlineStatus entries for status '{
                    status.name}': {
                    str(e)}")
            return False
    ...
    def delete_status_for_all_nightlines(cls, status: "Status") -> bool:
        ...
        logger.debug(
            f"Deleting NightlineStatus entries for status: '{
                status.name}'")
        ...
        try:
            ...
            rows_deleted = NightlineStatus.query.filter_by(
                status_id=status.id).delete()

            if rows_deleted > 0:
                db.session.commit()
                logger.info(
                    f"Successfully deleted '{rows_deleted}' NightlineStatus entries for status: '{
                        status.name}'")
                return True
            else:
                logger.warning(
                    f"No NightlineStatus entries found for status: '{
                        status.name}'")
                return False
        except SQLAlchemyError as e:
            db.session.rollback()
            logger.error(
                f"Error deleting NightlineStatus entries for status: '{
                    status.name}': {
                    str(e)}")
            return False
    ...
    def delete_statuses_for_nightline(cls, nightline: "Nightline") -> bool:
        ...
        try:
            ...
            # nightline_id
            rows_deleted = NightlineStatus.query.filter_by(
                nightline_id=nightline.id).delete()

            if rows_deleted > 0:
                db.session.commit()
                logger.info(
                    f"Successfully deleted '{rows_deleted}' NightlineStatus entries for nightline: '{
                        nightline.name}'")
                return True
            else:
                logger.warning(
    ...
    def update_instagram_story(
            cls, nightline: "Nightline", status: "Status", instagram_story: bool) -> bool:
        """Update the instagram_story value for a specific nightline and status."""
        logger.debug(
            f"Updating instagram_story for nightline: '{
                nightline.name}' and status: '{
                status.name}' to '{instagram_story}'")

        try:
            nightline_status = NightlineStatus.query.filter_by(
                nightline_id=nightline.id, status_id=status.id).first()
