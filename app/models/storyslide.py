...
class StorySlide(db.Model):
    ...
    def __save_story_slide_file(cls, file: FileStorage, nightline_status: "NightlineStatus", overwrite=False) -> Optional[os.PathLike]:
        ...
        if existing_file_path:
            if overwrite:
                logger.debug(
                    f"Replacing the already existing story slide: '{existing_file_path}' for status: '{nightline_status.status.name}'"
                )
    ...
    def update_story_slide(cls, file: FileStorage, nightline_status: "NightlineStatus") -> Optional["StorySlide"]:
        ...
        try:
            ...
            db.session.rollback()
            logger.error(
                f"Error creating StorySlide for status: '{nightline_status.status.name}' of nightline: '{nightline_status.nightline.name}': {e}"
            )
            return None
    ...
    def remove_story_slide(cls, nightline_status: "NightlineStatus") -> bool:
        ...
        try:
            ...
            logger.info(
                "for status: '{nightline_status.status.name}' of nightline: '{nightline_status.nightline.name}' removed successfully"
            )
            return True
        except Exception as e:
            logger.error(
                f"Error removing StorySlide for status: '{nightline_status.status.name}' of nightline: '{nightline_status.nightline.name}': {e}"
            )
            return False
